...
from app.services.report import build_monthly_csv, monthly_report_filename
...
@router.get("/report/monthly", dependencies=[Depends(require_role("HQ_ADMIN"))])
def monthly_report(year: int, month: int, session: Session = Depends(get_session)):
    """선택한 연·월에 마감된 정산서를 모은 회계 리포트 CSV (HQ) — plan_phase4.6."""
    if not (1 <= month <= 12):
        raise HTTPException(status_code=400, detail="month는 1~12 사이여야 합니다.")
    csv_text = build_monthly_csv(session, year, month)
    fname = monthly_report_filename(year, month)
    return Response(
